...
{
public:
    HadronisEngine(const std::string &weight_path,
                   float cutoff,
                   int max_neighbors,
                   int n_threads)
    ...
    py::array_t<float> predict(
        py::array_t<int, py::array::c_style | py::array::forcecast> atomic_numbers,
        ...
        py::array_t<int, py::array::c_style | py::array::forcecast> batch)
    {
        ...
        const ssize_t n_atoms = atomic_numbers.shape(0);
...
PYBIND11_MODULE(_lowlevel, m)
{
    py::class_<HadronisEngine>(m, "HadronisEngine")
        .def(py::init<const std::string &, float, int, int>(),
             py::arg("weight_path"),
             py::arg("cutoff") = 5.0f,
             py::arg("max_neighbors") = 64,
             py::arg("n_threads") = 1)
        .def("predict", &HadronisEngine::predict,
